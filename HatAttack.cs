...
public class HatAttack : MonoBehaviour {
    ...
    void setComponents()
    {
        ...
        if (!LaurelScript.facingRight) negation = -1; //for leftwards transform-based movement
        else if (LaurelScript.facingRight) { negation = 1; bounceback = -bounceback; }
        flyingBack = false;
        midAirStop = false;
        stopPassed = false;
        momentumPower /= 10f;
