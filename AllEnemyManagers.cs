...
{
    /* This class is to transfer data into the next scene, for example to show killcount in the Game Over / Elevator Scene 
    ...
    public string createListOfEnemiesKilled() //for console
    {
        for (int manager = 0; manager <= enemyManagers.Length - 1; manager++)
        {
            allKills += kills[manager].ToString() + " " + names[manager] + " killed, ";
        }
        return allKills;
    }
}